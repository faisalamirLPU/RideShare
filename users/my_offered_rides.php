...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_status'])) {
    ...
    if (in_array($new_status, ['pending', 'confirmed', 'cancelled'])) {
        $update = $conn->query("UPDATE bookings SET booking_status = '$new_status' WHERE id = $booking_id");
...
// Fetch ride offers (bookings on driver’s rides)
$sql = "SELECT b.id AS booking_id, b.booking_status, b.seats_booked, u.name AS passenger_name,
               r.pickup_location, r.drop_location, r.travel_date, r.travel_time, r.fare
        FROM bookings b
...
<head>
  <meta charset="UTF-8">
  <title>My Ride Offers - RideShare</title>
...
<body class="text-white min-h-screen bg-black bg-opacity-50 backdrop-blur-md">
  ...
  <div class="max-w-5xl mx-auto px-6 py-12">
    <h2 class="text-4xl font-bold mb-8">🚘 My Ride Offers</h2>
      ...
      <ul class="space-y-6">
        <?php while($row = $result->fetch_assoc()): ?>
          <li class="bg-white/10 border border-white/20 rounded-2xl p-6 shadow-md">
            <div><strong>Passenger:</strong> <?php echo htmlspecialchars($row['passenger_name']); ?></div>
            ...
            <div><strong>Seats Booked:</strong> <?php echo $row['seats_booked']; ?></div>
            <div><strong>Status:</strong> <?php echo ucfirst($row['booking_status']); ?></div>

            <form method="POST" class="mt-4 space-y-2 md:flex md:items-center md:space-x-4">
              <input type="hidden" name="booking_id" value="<?php echo $row['booking_id']; ?>">
              <select name="new_status" class="bg-white text-black px-4 py-2 rounded-md">
              ...
              <button type="submit" name="update_status" class="bg-yellow-500 hover:bg-yellow-600 text-black px-4 py-2 rounded-md">
                Update Status
              </button>
            </form>
          </li>
    ...
    <?php else: ?>
      <p class="text-gray-300 text-lg">No bookings received for your rides yet.</p>
    <?php endif; ?>
  </div>
